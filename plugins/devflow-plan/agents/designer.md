...
- **KNOWLEDGE_CONTEXT** (optional): Compact index of active ADR/PF entries for this worktree. `(none)` when absent. Use `devflow:apply-knowledge` to Read full bodies on demand.
...
3. **Apply Knowledge** — Follow `devflow:apply-knowledge` to scan the index, Read relevant bodies on demand, and cite `applies ADR-NNN` / `avoids PF-NNN` in findings. Skip when `KNOWLEDGE_CONTEXT` is empty or `(none)`.
