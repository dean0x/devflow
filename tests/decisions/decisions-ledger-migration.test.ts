...
// ---------------------------------------------------------------------------
// AC-F11: gitignore template — anchored ledger tracked, log + archive gitignored
// ---------------------------------------------------------------------------

describe('gitignore template — AC-F11', () => {
  it('re-includes decisions-ledger.jsonl in the template', () => {
    const content = getDevflowGitignoreContent();
    expect(content).toContain('!decisions/decisions-ledger.jsonl');
  });

  it('does NOT re-include decisions-log.jsonl (remains gitignored)', () => {
    const content = getDevflowGitignoreContent();
    expect(content).not.toContain('!decisions/decisions-log.jsonl');
  });

  it('does NOT re-include decisions-log.archive.jsonl (remains gitignored)', () => {
    const content = getDevflowGitignoreContent();
    expect(content).not.toContain('!decisions/decisions-log.archive.jsonl');
  });
});

// ---------------------------------------------------------------------------
// sync-devflow-gitignore-v3 migration: adds ledger line idempotently
// ---------------------------------------------------------------------------

describe('sync-devflow-gitignore-v3 migration', () => {
  it('adds !decisions/decisions-ledger.jsonl to an existing .devflow/.gitignore', async () => {
    const devflowDir = path.join(projectRoot, '.devflow');
    await fs.mkdir(devflowDir, { recursive: true });
    const gitignorePath = path.join(devflowDir, '.gitignore');

    // Simulate an older template without the ledger re-include
    const olderContent = `*
!.gitignore
!decisions/
...
`;
    await fs.writeFile(gitignorePath, olderContent, 'utf-8');

    // Run the migration manually using the registry shape
    const { MIGRATIONS } = await import('../../src/cli/utils/migrations.js');
    const migration = MIGRATIONS.find(m => m.id === 'sync-devflow-gitignore-v3');
    expect(migration).toBeTruthy();
    const result = await (migration as { run: (ctx: { scope: 'per-project'; devflowDir: string; memoryDir: string; projectRoot: string }) => Promise<{ infos: string[]; warnings: string[] }> }).run({
      scope: 'per-project',
      devflowDir: path.join(path.dirname(devflowDir), '.devflow'),
      memoryDir: path.join(path.dirname(devflowDir), '.devflow', 'memory'),
      projectRoot,
    });

    expect(result.infos).toHaveLength(1);
    expect(result.infos[0]).toContain('decisions-ledger.jsonl');

    const updated = await fs.readFile(gitignorePath, 'utf-8');
    expect(updated).toContain('!decisions/decisions-ledger.jsonl');
  });

  it('is idempotent when ledger line already present', async () => {
    const devflowDir = path.join(projectRoot, '.devflow');
    await fs.mkdir(devflowDir, { recursive: true });
    const gitignorePath = path.join(devflowDir, '.gitignore');

    // Write the full current template (already has the ledger line)
    const { getDevflowGitignoreContent: getContent } = await import('../../src/cli/utils/project-paths.js');
    await fs.writeFile(gitignorePath, getContent(), 'utf-8');
  ...
  it('is a no-op when .devflow/ directory does not exist', async () => {
    const emptyRoot = path.join(tmpDir, 'empty-project');
    await fs.mkdir(emptyRoot, { recursive: true });
    ...
    const result = await (migration as { run: (ctx: { scope: 'per-project'; devflowDir: string; memoryDir: string; projectRoot: string }) => Promise<{ infos: string[]; warnings: string[] }> }).run({
      scope: 'per-project',
      devflowDir: path.join(emptyRoot, '.devflow'),
      memoryDir: path.join(emptyRoot, '.devflow', 'memory'),
...
// ---------------------------------------------------------------------------
// CJS parity: getDevflowGitignoreContent TS == CJS
// Already tested in project-paths.test.ts, but verify the new ledger line
// ---------------------------------------------------------------------------

describe('gitignore CJS parity for decisions-ledger.jsonl', () => {
  it('CJS getDevflowGitignoreContent includes !decisions/decisions-ledger.jsonl', () => {
    const cjsPaths = require(path.join(ROOT, 'scripts/hooks/lib/project-paths.cjs')) as {
      getDevflowGitignoreContent: () => string;
    };
    const cjsContent = cjsPaths.getDevflowGitignoreContent();
    expect(cjsContent).toContain('!decisions/decisions-ledger.jsonl');
    // CJS and TS must be identical
    expect(cjsContent).toBe(getDevflowGitignoreContent());
  });
});
