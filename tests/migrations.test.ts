...
describe('purge-stale-memory-markers-v1 migration', () => {
  ...
  it('appears after purge-orphaned-dream-commit-hook-v1 in MIGRATIONS array', () => {
    const dreamCommitIdx = MIGRATIONS.findIndex(m => m.id === 'purge-orphaned-dream-commit-hook-v1');
    const memoryIdx = MIGRATIONS.findIndex(m => m.id === 'purge-stale-memory-markers-v1');
    expect(dreamCommitIdx).toBeGreaterThanOrEqual(0);
    expect(memoryIdx).toBeGreaterThan(dreamCommitIdx);
  });
