...
describe('check', () => {
  ...
  const CASES: ReadonlyArray<readonly [string, 'tp' | 'block' | 'exceptions', string, number]> = [
    ...
    // D-TP-SCENARIO: a scenario never carries a reference, a mention, a link or markup.
    ['a scenario with a closing keyword', 'tp', `${tpLine(1, 'Closes #12', 'ci')}\n`, 5],
    ['a scenario with an issue URL', 'tp', `${tpLine(1, 'fixes https://github.com/o/r/issues/1', 'ci')}\n`, 5],
    ['a scenario with a mention', 'tp', `${tpLine(1, 'ping @user', 'ci')}\n`, 5],
    ['a scenario with an img tag', 'tp', `${tpLine(1, '<img src=x>', 'ci')}\n`, 5],
    ['a scenario with a code span', 'tp', `${tpLine(1, 'run `npm test`', 'ci')}\n`, 5],
    ['a block whose scenario closes an issue', 'block', `${BLOCK_START}\n## Test Plan\n${tpLine(1, 'Closes #12', 'ci')}\n${BLOCK_END}\n`, 5],
    ['a block whose scenario mentions a user', 'block', `${BLOCK_START}\n## Test Plan\n${tpLine(1, 'ping @user', 'ci')}\n${BLOCK_END}\n`, 5],
