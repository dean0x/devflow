...
// ─── Part G: Init sentinel integration ──────────────────────────────────────

describe('init sentinel management', () => {
  let tmpDir: string;

  beforeEach(() => { tmpDir = mkTmpDir(); });
  afterEach(() => { fs.rmSync(tmpDir, { recursive: true, force: true }); });

  it('--no-memory creates .working-memory-disabled sentinel via init logic', async () => {
    // Test the sentinel file path logic in isolation
  ...
  it('--memory removes .working-memory-disabled sentinel', async () => {
    const memDir = path.join(tmpDir, '.memory');
    const sentinel = path.join(memDir, '.working-memory-disabled');
    fs.mkdirSync(memDir, { recursive: true });
    fs.writeFileSync(sentinel, '', 'utf-8');
    // Simulate enable: unlink
    try { fs.unlinkSync(sentinel); } catch { /* no-op */ }
    expect(fs.existsSync(sentinel)).toBe(false);
  });

  it('--no-learn creates .learning-disabled sentinel via init logic', async () => {
    const memDir = path.join(tmpDir, '.memory');
  ...
  it('double-disable is idempotent (sentinel already exists)', async () => {
    const memDir = path.join(tmpDir, '.memory');
    const sentinel = path.join(memDir, '.working-memory-disabled');
    fs.mkdirSync(memDir, { recursive: true });
    fs.writeFileSync(sentinel, '', 'utf-8');
  ...
  it('double-enable is idempotent (sentinel already gone)', async () => {
    // Unlink non-existent sentinel — no error
    const sentinel = path.join(tmpDir, '.memory', '.working-memory-disabled');
    expect(() => {
      try { fs.unlinkSync(sentinel); } catch { /* expected — doesn't exist */ }
    }).not.toThrow();
  });
});

// ─── Part A: Shell hook list includes session-start-context ──────────────────

describe('shell hook syntax: session-start-context', () => {
  it('session-start-context passes bash -n', () => {
    const scriptPath = path.join(HOOKS_DIR, 'session-start-context');
    if (!fs.existsSync(scriptPath)) {
      // Will fail in RED phase — expected
      throw new Error(`session-start-context does not exist at ${scriptPath}`);
    }
    expect(() => {
