...
| **PreCompact** | Before context compaction | Backs up git state + WORKING-MEMORY.md snapshot to `backup.json`. Stamps line 1 of WORKING-MEMORY.md with the HEAD SHA (bootstrap guard: 40-hex gate prevents unstamped bootstrap at session start, fixing the "synced @ unknown" pre-compact state). |
