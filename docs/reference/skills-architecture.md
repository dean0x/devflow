...
**Generated references.** A skill's `references/` directory can also receive files the build emits. `devflow:git` is the one skill that does: `npm run build:mds` compiles the `.mds` reference modules in `src/assets/mds/` into `dist/skills/git/references/**`, and `devflow init` overlays that tree onto the installed skill directory. Generated files are never written into `src/`, and the overlay converges rather than merges — a file under `references/tracker/**` or `references/pr/**` that the build manifest does not name is removed on the next install, so a shadowed skill cannot substitute its own mechanics. `references/pr/` (the nine PR-host operation bodies) converges the same way `references/tracker/` does, but installs unconditionally under every provider rather than being scoped to the selection. Four cross-cutting documents — `decision-markers.md`, `learn-conventions.md`, `publication-gate.md` and `trust-rule.md` — are generated into the references root under every provider too. That root is overlaid but never pruned: hand-authored files beside the generated ones (`github-api.md`, `patterns.md`, …) are never touched, and a retired cross-cutting document keeps its installed copy until the skill directory is replaced.
