...
- **decisions-format** — Format spec for `.devflow/decisions/decisions.md` and `pitfalls.md` (entry format, lock protocol). Consumed by the background sidecar processor via `json-helper.cjs decisions-append`. Not distributed to plugins per D9.
