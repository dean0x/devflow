...
description: "Use when modifying dream hooks, background maintenance, marker lifecycle, memory/learning/decisions/knowledge processing, or curation. Keywords: dream, hooks, background processor, merge-observation, decisions-append, marker, .processing, SessionStart, dream-capture, dream-evaluate."
...
The Dream system is a three-hook pipeline that captures session context, evaluates what background work is needed, and coordinates a single background LLM agent to do that work. The hooks are installed from `scripts/hooks/` (source of truth) to `~/.devflow/scripts/hooks/` at `devflow init` time. The background agent spec lives at `shared/agents/dream.md`.

The system is explicitly split into two layers: **plumbing** (hooks, locks, marker files, atomic writes, JSONL logs) and **LLM** (all detection, semantic matching, content authoring, curation judgment). No `claude -p` subprocess for individual features — one background Dream agent handles all pending task types sequentially.

## System Context

Five task types can be pending at any session start: `memory`, `learning`, `decisions`, `knowledge`, `curation`. Each gets its own marker file in `.devflow/dream/`. The three hooks are:

| Hook | Trigger | Role |
|------|---------|------|
| `dream-capture` | Stop (per turn) | Append assistant turn to queue; write `memory.json` marker when throttle (120s) expires |
| `dream-evaluate` | SessionEnd | Source eval-* modules; write per-session markers for learning/decisions/knowledge/curation |
| `session-start-context` | SessionStart | Recover stale `.processing`, collect pending markers, emit DREAM MAINTENANCE directive |

The main model's only action is a single `Agent(subagent_type="Dream", run_in_background: true)` call. The Dream agent claims, processes, and deletes each marker.
...
session-start-context reads:
  ...
  - dream_collect_tasks()                        ← collects .json marker types
  - raw-queue bridge                             ← adds memory task if .pending-turns.jsonl non-empty
  - emits DREAM MAINTENANCE directive            ← instructs model to spawn Dream agent
...
**Multi-marker merge**: When multiple `{type}.{session}.processing` files exist for one type, the Dream agent reads them all and unions their payloads before processing: learning/decisions concatenate signal strings and union `existingObservationIds`; knowledge unions `staleSlugs`; memory and curation use single markers only. Input cap: last 30 userSignals/dialog-pairs (bounds token cost per run).
...
Section 2 of `session-start-context` includes a fallback that adds the `memory` task type even when no `memory.json` marker file exists — if `.pending-turns.jsonl` is non-empty and memory is enabled. This handles the race window where `dream-capture` wrote turns to the queue but the session ended before the 120s marker-write throttle expired. Without this bridge, those captured turns would sit in the queue indefinitely.

## Queue: Memory Channel

`dream-capture` (Stop hook) appends assistant turns to `.devflow/memory/.pending-turns.jsonl` (JSONL, each line `{role, content, ts}`). The Dream agent claims it atomically by renaming to `.pending-turns.processing`. If the session crashes mid-processing, `dream_recover_stale` in the next SessionStart renames `.processing` back to `.jsonl` — but only when no fresh `.jsonl` already exists (non-clobber guard, D56c).
...
`dream-evaluate` uses `scripts/hooks/lib/transcript-filter.cjs` to extract two channels from the session transcript before writing markers:
...
- Caller holds `.devflow/dream/.reinforce.lock` (mkdir-based) EXTERNALLY — the lock is acquired by the Dream agent around the Bash call, then released. Never held across tool calls.
...
**`read-dream <file> <field>`** — reads a field from a dream JSON marker file; returns `[]` on any error. Routes to `./lib/dream-ops.cjs`.

## LLM-vs-Plumbing Principle

The boundary is strict:

| Plumbing (deterministic code) | LLM (background Dream agent) |
|-------------------------------|-------------------------------|
...
No `claude -p` subprocess is spawned for individual features. The Dream agent spec (`shared/agents/dream.md`) is the single spec the LLM processor follows.

## Staleness Signal (staleness.cjs)

`scripts/hooks/lib/staleness.cjs` annotates learning/decisions log entries with `mayBeStale: true` + `staleReason` when file paths extracted from `details`/`evidence` fields no longer exist on disk. The Dream agent runs this before reinforcing (learning task) or selecting deprecation candidates (curation task). Staleness is a signal to the LLM, not an automatic action:
...
The CLI display surface for staleness (per-observation review flags, `⚠ flagged` output lines) has been removed. Staleness is now a background signal only.

## Curation (eval-curation)

`eval-curation` (sourced by `dream-evaluate`) writes a `curation.{session}.json` marker, throttled to once every 7 days via `.devflow/dream/.curation-last` epoch file. The curation task in the Dream agent:
...
The primary source of truth for feature enabled-state is `.devflow/dream/config.json`. All three hooks (`dream-capture`, `dream-dispatch`, `dream-evaluate`) and `session-start-context` include a transitional fallback that reads `.devflow/sidecar/config.json` when the dream config is absent — marked with `# dream-fallback: REMOVE after one release`. This handles projects that haven't run `devflow init` since the sidecar→dream rename.
...
- **Calling `decisions-append` during curation** — it acquires `.decisions.lock` internally; calling it while holding that lock deadlocks. Use Edit tool for deprecation (dream.md explicit warning).
- **Holding a lock across tool calls** — the Dream agent's lock lifecycle must be: acquire → single Bash → release. Never span multiple tool calls under one lock.
- **Spawning multiple background agents** — the main model makes exactly one `Agent(subagent_type="Dream", run_in_background: true)` call. The Dream agent handles all task types sequentially inside one agent.
- **Assuming `dream-dispatch` injects the DREAM directive** — `dream-dispatch` is capture-only (UserPromptSubmit); the DREAM MAINTENANCE directive is emitted by `session-start-context` (SessionStart) (ADR-009).
...
- **Writing artifact content in deterministic scripts** — memory, observations, ADR/PF bodies, and knowledge bases must be authored by the LLM Dream agent; plumbing scripts handle only structural writes (ADR-008).
...
- **set -e / no-abort discipline** — `dream-capture` and `dream-evaluate` use `set -e`. However, `session-start-context` intentionally omits `set -e` because its three sections (1.5 decisions TL;DR, 1.75 learned behaviors, 2 dream pending-work) are independent — a failure in Section 1.5 must not prevent Section 2 from running. Never add `set -e` to `session-start-context`.
...
- `scripts/hooks/dream-evaluate` — SessionEnd hook; orchestrator sourcing eval-* modules (eval-learning, eval-decisions, eval-knowledge, eval-curation, eval-reinforce, eval-helpers)
- `scripts/hooks/eval-curation` — sourced by dream-evaluate; writes `curation.{session}.json` marker on 7-day throttle (D58); `.curation-last` in `.devflow/dream/`
...
- `scripts/hooks/dream-collect-tasks` — sourced helper; collects pending `.json` markers into `_DREAM_TASKS`; COLLECT_LIMIT=50 FIFO; deletes disabled-feature markers
- `scripts/hooks/session-start-context` — SessionStart hook (no set -e); three independent sections: 1.5 decisions TL;DR, 1.75 learned behaviors, 2 dream spawn directive (with raw-queue bridge)
...
- `shared/agents/dream.md` — full Dream agent spec: claim, heartbeat, multi-marker merge, task types, error discipline
