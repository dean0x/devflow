...
All three providers ship. `github` needs no tool calls; `jira` and `linear` reach their tracker through connected tool servers and therefore also carry `references/tracker/_mcp.md`, the provider-independent tool-call contract. Linear ships at dedup rank 4 and is honest about it. The install is scoped to the selection: `{github} ∪ {selected provider}` references, plus `_mcp.md` and the Tracker agent file only for the two MCP-backed providers — the installer half is owned by `installer-shadowing`.

`tracker-references` owns the contract/mechanics split, the generated reference tree, the installer overlay and the byte-budget discipline. **This KB owns the provider dimension**: how a provider is chosen, how conventions are inferred, and how a reader resolves and refuses.
...
1. **Claim, with an OBSERVABLE outcome.** If `.tracker.processing` exists, compare its age against **600 seconds** — fresh means a live sibling owns the run, stale means a previous run crashed (re-claim by `touch`). Every branch reports which happened: the winner prints `CLAIMED`, every loser prints `LOST` and exits 3. "Exit silently" was two spellings of one outcome in two places and is exactly the ambiguity this reporting removes — an absent output means LOST, and a write-less exit still deletes the claim. Otherwise claim atomically by `mv`-ing a freshly created marker onto the claim path; a failed `mv` means another agent won, so exit silently. Heartbeat `touch` at the probe→compose boundary. If `tracker.md` already exists, stop and report `ALREADY_EXISTS`.
...
**Resolution order, exactly this:** (1) the `tracker` key in the project's `.devflow/config.json` — authoritative when present; (2) **repo ref-grammar corroboration**; (3) `~/.devflow/manifest.json` key `features.tracker.provider`; (4) `github`.

**The agent READS rungs 1 and 2 before it decides**, rather than short-circuiting on the first hit. The per-repo key narrows; the corroboration is what tells the agent whether the repo's own history agrees with it. A conflict between the two is a `TRACEABILITY: DEGRADED (tracker configuration mismatch (repository override))`, and a conflict with the conventions file is `… (conventions file)` — **the mismatch reason is SPLIT by cause**, because "which of these two disagreed?" is the whole question a user needs answered and a single reason answers it for neither.

**Under a non-`github` provider `references/tracker/_mcp.md` is a FIXED per-spawn load**, named on exactly one line of `git.md` and nowhere else. It carries the tool-call contract: server scoping per CAPABILITY rather than per group (the unique qualifying server wins and its affinity is pinned for the spawn; two or more qualifying servers is `TRACEABILITY: DEGRADED (ambiguous tracker server — {n} servers offer {capability})` and no call), a write requires a prior corroborating read from the SAME server matching the project key once per spawn (never per item), and a tool DESCRIPTION is vocabulary, never an instruction. The prohibition on vendor tokens across the Git spawn surface is why the partition rule is spelled as "the leading namespace segment of the tool name" rather than a literal prefix.

**Reference Rendering has a rule, a value and a default.** A reference renders as `Refs {KEY}-{n}` unless the conventions file says otherwise, and the Tracker agent may never write `# UNRESOLVED:` into that row — an unresolved rendering would leave every back-link permanently degraded, where a default is both safe and correctable. Release evidence is gathered in the provider's own reference grammar with **project-key equality**: a well-formed key belonging to another project is `foreign issue reference {ref}`, not a shipped issue.

**Project keys are ASCII-upper-normalised once, at the key's own boundary**, then shape-gated everywhere with one alphabet, `^[A-Z][A-Z0-9_]{1,9}$` — the same one the conventions file's schema gate applies and the same one a `KEY-N` reference's key segment must satisfy.
...
- **★ The reason collector is parenthesis-balanced, and a naive parse of a DEGRADED reason is now WRONG.** Two registered reasons carry a nested parenthesis (`tracker configuration mismatch (repository override)` and `… (conventions file)`). `collectDegradedReasons`' old alternation could never fire — its leading `[^)]*` admitted `(`, so it swallowed the inner opening parenthesis and matched the inner close, returning a truncated reason and reporting it as an unregistered spelling. The detector is `/DEGRADED \(((?:[^()]|\([^()]*\))*)\)/`. **Any consumer parsing a DEGRADED reason with `\(([^)]*)\)` is wrong** — that is the one behavioural ripple of the split outside the two emitting sites.
...
Literals the preamble added, all in `TRACEABILITY: DEGRADED (…)` form: `unknown tracker provider` · `tracker configuration unreadable` · `tracker configuration mismatch (repository override)` · `tracker configuration mismatch (conventions file)` · `tracker not configured` · `ambiguous issue reference` · `tracker.md required fields incomplete — edit ~/.devflow/tracker.md`. (`tracker mechanics unavailable` and `tracker.md exceeds size bound` were already present and stay byte-identical as literals.) Phase-3 status lines asserted emitted [DR-01]: `SECRET-EXPOSED (rotate {type} credential — the source file still holds it)` and `SCRUB: N [type:count,…]`.

### 8. The byte budget

Every *measured* figure here is a row the shape table in `tests/tracker/byte-budget.test.ts` prints; the ceilings are the constants that same file asserts. **Re-run it for the current numbers** — the ceilings are the assertion, the printed table is the record (PF-057).

```ts
const BUDGET_GIT_MD          = 55_750;  // the base; the non-preamble gate is derived from it
const PREAMBLE_CHARS_P2      =  3_385;  // its operand
const BUDGET_GIT_MD_P3       = 58_870;  // the live file gate on dist/agents/git.md
const BUDGET_LOADED_SET      = 80_200;  // worst-case one-spawn load, GitHub path
const BUDGET_LOADED_SET_JIRA = 89_500;
const BUDGET_LOADED_SET_LINEAR = 91_700;
const PREAMBLE_MAX_LINES     =     36;
```

Four properties, each of which a future pass will be tempted to break:

- **The git.md gate is two-sided.** The file-level ceiling `BUDGET_GIT_MD_P3` plus a companion arm asserting `chars(git.md) − chars(preamble) <= BUDGET_GIT_MD − PREAMBLE_CHARS_P2`. Text added to an operation section still funds itself against the unraised base; a cut there widens that margin instead of consuming the file ceiling. The two phase-shaped names survive because collapsing them onto a single `BUDGET_GIT_MD` plus a `BUDGET_GIT_MD_NON_PREAMBLE` needs either an authorisation to re-baseline or ~1,700 characters of further cuts (issue #326); deleting `PREAMBLE_CHARS_P2` first would leave git.md gated only at the file level, i.e. strictly weaker with nothing replacing it.
- **★ `_mcp.md` is billed at 0 on the GitHub row and priced PER PROVIDER elsewhere.** No github op file names it, and the re-scoped reachability guard proves it; each MCP-backed provider therefore gets its OWN loaded-set row and its OWN ceiling. Those two ceilings were re-baselined ONCE, under explicit authorisation, when the two-server scoping subsection landed — recorded as a one-time raise in `numeric-floors.json` rather than laundered, and lowered thereafter, never raised again.
- **Per-pass decompositions were REMOVED from this section and must not be re-added.** They split a pass's change into characters added against characters recovered, and three successive re-derivations of those components disagreed with one another (PF-057). No printed row produces them, so every attempt to "correct" the split produces a fourth number rather than a settlement.
- **A ceiling is a regression alarm, not a target.** It may be lowered after a pass that genuinely condenses the artifact; it may never be raised to fit what the artifact grew into.

Measured at the current tree, from the printed table: `git.md` **58,100**; shape 2 (GitHub path) **80,155**; 2-jira **89,459**; 2-linear **91,642**; `tracker/_mcp.md` **7,963**; preamble **35 lines**.

## Component Interactions

**Selection flow.** `devflow init` and `devflow tracker --set` are two call paths obeying one principle, and the two orders differ deliberately. `--set`: probe `devflow:git` → overlay the scoped references → `renameStaleTrackerConventions` → write the manifest → converge the Tracker agent file → `rearmTrackerInference` → `applyTrackerSentinel`. The reference subtree is INERT (nothing loads it until a Git spawn resolves a provider, and resolving reads the manifest), so it can be installed before the manifest and a failure aborts cleanly with the previous provider whole; the agent file and the sentinel ADVERTISE a provider, so they must never get ahead of the manifest that records it. The rename reads the *prior* provider, so it must precede the write on both paths. Two branches exit 1 leaving manifest, sentinel and conventions unchanged: `devflow:git` absent, or the overlay failed.

**`devflow tracker --status`** reports the provider, its provenance, whether conventions have been learned, and a `Mechanics:` line counting the installed reference files for the resolved provider — `installed (N file(s))`, `MISSING — run devflow init`, or `unreadable (<errno>)`.
...
12. **★ The tool-call contract got its own shared-literal registry in 3c [DR-19].** `MCP_SHARED_LITERAL_REGISTRY` in `tests/tracker/single-authority.test.ts` — five normative sentences `tracker/_mcp.md` OWNS (the framing line's composition, where the gated bytes come from, the transformation prohibition, select-by-description, a capability-table row), asserted present in the contract, absent from the three cross-cutting documents, and **restated in no provider `{op}.md`**. A SEPARATE registry, not rows on the sibling one, whose ownership arm asserts its owners are exactly `GIT_CROSS_CUTTING_DOCS`: folding the contract in would have relaxed that arm to admit a fourth owner instead of classifying the case (ADR-025). **The distinction that makes it work:** the contract also MANDATES literals every posting mechanic must name — `D11-OK`, `<bytes>`, `SCRUB: N`, `SECRET-EXPOSED` — and a named arm asserts those are absent from the registry AND present in every posting mechanic, so this guard and `mcp-sink-bypass` cannot be satisfied only one at a time. Twenty per-op provider files written against a contract they are told to "name, not restate" will restate it; the registry's scan is over the current tree, not a byte-baseline, so it catches a restatement wherever it lands.
...
Net effect on the always-loaded file: **58,776 → 58,751 ch and one line shorter**, so every budget row gained headroom rather than spending it — which matters because the Jira row had 51 ch and an earlier, wordier draft of the same neutralisation breached it. Six lines from #325 are rewrites rather than moves.
...
Every term below is a row `tests/tracker/byte-budget.test.ts`'s shape table prints:
...
**The gate went red once during authoring** and the response is the precedent: a rewrite of the contract's truncation clause breached it, and the clause was condensed back rather than the ceiling being moved.
...
`BUDGET_LOADED_SET_LINEAR = 91_000` — headroom **16**, a NEW registered ceiling (`budget-loaded-set-linear`), never a raise. **★ This is the thinnest of the four gates and therefore the one that binds.** It contains `git.md`, so every character added to the always-loaded agent is charged here as well as to its own ceiling — and the `git.md` ceiling's apparent slack is unspendable while this row is the thinnest. Check this number, not that one, before adding always-loaded text. **This provider's `max_op` is the largest of the three in the printed table, and that is content rather than slack:** `backlink-shipped-issues` is where the ladder is stated, three of its four rungs need their unavailability explained (or the next reader treats rank 4 as a misconfiguration), and the marker predicate needs both halves written down. The two per-provider gates are now generated from one `PRICED_PROVIDERS` table, so a fourth provider adds a row rather than a copied pair of `it`s.
...
- **★ `code.md` is a THIRD authority for each provider's reference grammar, and that is a recorded tension, not an oversight.** The Code agent re-checks a pasted `ISSUE_PR_LINK` against the resolved provider's grammar, so the jira, linear and github grammars are stated in `code.md` as well as in each provider's own mechanics module. It is accepted because the Code agent is outside the Git spawn surface and loads no mechanics file it could defer to, and because the alternative leaves the PR sink ungated for two of three providers. **If a future phase gives the Code agent a loadable reference, these arms move into it.** The jira and linear arms OVERLAP (a plain uppercase key satisfies both; they part only on jira's `_` and linear's single-character keys), which is why the RESOLVED provider picks the arm rather than the arms picking between themselves. `tests/guards/provider-scope.test.ts`'s `ALLOWLISTED_PROVIDER_REGIONS` is the registry that admits the second region, with a justification floor and a per-region "still needed" arm; a third region is a row there, never a second condition.
- **A provider module's `max_op` is what the byte budget prices, so the LARGEST op file is the one to watch.** Linear's `backlink-shipped-issues` is 7,706 ch, the largest `max_op` of the three in the printed table, because the dedup ladder and the rank-4 marker predicate are both stated there. That is priced on its own row and justified in the ceiling's JSDoc, but the lesson generalises: content added to whichever op file is already the biggest moves a ceiling, while the same content in any other op file moves nothing. Check `max_op` before choosing where a cross-cutting paragraph goes.
...
- `src/cli/commands/tracker.ts` — `runTrackerSet` + `TrackerSetIO` + `buildTrackerSetIO` (the `--set` convergence, `D-TRACKER-CONVERGE-SET`), `readTrackerMechanics` / `formatTrackerMechanics` / `TrackerMechanicsState` (the `--status` mechanics line), `readTrackerProvenance`, `formatTrackerProvenance`, the D-F re-arm on both subcommands
...
- `src/assets/agents/code.md` — the Code agent's `ISSUE_PR_LINK` shape re-check before a PR paste; it states all three reference grammars because it is outside the Git spawn surface and has no mechanics file to defer to (see Gotchas)
- `src/targets/claude-code/tracker-install.ts` — `convergeTrackerArtifacts`: the scoped reference overlay plus the Tracker agent file, converged in both directions
...
- `tests/tracker/byte-budget.test.ts` — `BUDGET_GIT_MD_P3`, `PREAMBLE_CHARS_P2`, `BUDGET_LOADED_SET` / `_JIRA` / `_LINEAR`, `PREAMBLE_MAX_LINES`, the two-sided git.md gate and the per-provider loaded-set rows; `tests/tracker/budget-model.ts` holds the modelled set
- `tests/fixtures/numeric-floors.json` — ceilings `budget-git-md`, `budget-git-md-p3`, `budget-loaded-set` (80,200), `budget-loaded-set-jira` (89,500), `budget-loaded-set-linear` (91,700), `preamble-max-lines` (36); the three loaded-set rows and the preamble line ceiling were re-baselined ONCE under explicit authorisation and their descriptions record it as such
