...
- `src/cli/commands/tracker.ts` — `resolveTrackerCliAction` (pure), `runTrackerSet` + `TrackerSetIO` + `buildTrackerSetIO` (the `--set` convergence, `D-TRACKER-CONVERGE-SET`), `readTrackerMechanics` / `formatTrackerMechanics` / `TrackerMechanicsState` (the `--status` mechanics line), `readTrackerProvenance`, `formatTrackerProvenance`, the D-F re-arm on both subcommands
