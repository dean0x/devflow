...
Every *measured* figure in this section is a row the shape table in `tests/tracker/byte-budget.test.ts` prints; the ceilings are the constants that same file asserts. Re-run it for the current numbers — the ceilings are the assertion, the printed table is the record.

| Printed row | ch |
|---|---|
| `git.md` chars | 58,782 |
| worst-case tracker spawn, GitHub path (shape 2) | 80,837 |

**Per-pass decompositions were REMOVED from this section and must not be re-added.** They split each pass's change to the always-loaded file into characters added against characters recovered, and three successive re-derivations of those components disagreed with one another (PF-057) — no printed row produces them, so every attempt to "correct" the split produced a fourth number rather than a settled one. The decisions those sentences carried are kept, in words: #325's neutralisation of the tracker ops' wording deleted a duplicated sentence, so the always-loaded file got SMALLER while gaining a provider; the alignment pass then funded its M2 rendering rule by deleting `backlink-shipped-issues`' restatement of the D4 rate-limit rung (M4); and the second alignment pass's issue-ref scoping landed wholly inside the preamble. **`BUDGET_GIT_MD_P3` was NOT raised for any of it** — a ceiling is re-derived downward or not at all, and nothing about these changes earns a lower one.

**Re-measure before spending, and read the LINEAR row, not this one.** Figures in this section had already drifted by the time the alignment pass measured them, which is why every "measured"/"headroom" number in `byte-budget.test.ts` and `numeric-floors.json` is re-derived from the printed table rather than carried forward. And the binding constraint is no longer the `git.md` ceiling: it is the **Linear loaded-set row**, because that row contains `git.md`. A character added to the always-loaded agent is a character added to all four gates, and the smallest margin decides.
...
- **The ≤70-line preamble ceiling was NOT added.** §14.10 called 70 "the honest number"; the re-derivation measured the preamble comfortably inside the `<= 40` already in place, so a `<= 70` assertion would be strictly *weaker* and would buy nothing. **It was not added in 3b, 3c or the alignment pass either** — it would be a raise wearing a new name. `PREAMBLE_MAX_LINES`' JSDoc records the measured line count beside the ceiling; only a few of the forty lines are left, so the next always-loaded rule either fits in one line or replaces one.
- **The P3 revision is spendable on the preamble ONLY, mechanically.** A companion gate asserts `chars(git.md) − chars(preamble) <= BUDGET_GIT_MD − PREAMBLE_CHARS_P2`; no row of the shape table prints either side, so the figures are not restated here — the gate's own failure message names them. **Text added to an operation section still funds itself against Phase 2's number**; the neutralisation and then the alignment pass's M4 cut both went the other way, so that margin is wider than Phase 2's rather than narrower.
