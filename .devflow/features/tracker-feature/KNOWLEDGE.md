...
| | Phase 2 | Phase 3a | Phase 3c |
|---|---|---|---|
| `git.md` chars | 55,664 | 58,776 | **58,751** |
| `git.md` **outside** the preamble | 52,279 | 52,279 — byte-identical | **52,254** |
| preamble chars / lines | 3,385 / 29 | **6,497 / 34** | 6,497 / 34 — unchanged |
| worst-case tracker spawn, GitHub path (shape 2) | 77,719 | 80,831 | **80,806** |

The 3c column moves DOWNWARD because #325's neutralisation of the tracker ops' wording deleted a duplicated sentence: the always-loaded file got smaller while gaining a provider. `BUDGET_GIT_MD_P3` stays at 58,870 (headroom 119) — a ceiling is re-derived downward or not at all, and nothing about this change earns a lower one on its own.
...
- **The ≤70-line preamble ceiling was NOT added.** §14.10 called 70 "the honest number"; the re-derivation says **34**. A `<= 70` assertion would be strictly *weaker* than the `<= 40` already in place. **It was not added in 3b or 3c either** — it would be a raise wearing a new name, and the preamble is still 34 lines.
- **The P3 revision is spendable on the preamble ONLY, mechanically.** A companion gate asserts `chars(git.md) − chars(preamble) <= BUDGET_GIT_MD − PREAMBLE_CHARS_P2` (= 52,365, measured 52,279 — Phase 2's own 86 ch of headroom). **Text added to an operation section still funds itself against Phase 2's number**; #325's neutralisation left 111 ch of headroom there rather than spending any.
...
3. **`tests/guards/mcp-sink-bypass.test.ts` is LIVE.** The `★ the live corpus is EMPTY at this boundary` assertion is deleted; in its place the corpus is asserted non-empty AND to hold at least one file spelling `{SCRUBBED_BODY}`, because every clause arm is an empty-difference assertion and a read-only tree would clear the first check. Two further strengthenings landed with it: `collectBypassSites` now runs over the live corpus (it had only ever run against its own seeds — PF-064), and `POSTING_VERBS` admits a SPACE separator, because the contract mandates selection by capability DESCRIPTION (*add comment*) rather than by tool name, so `[_-]?` matched every tool name and no compliant mechanics file.
