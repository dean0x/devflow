...
`dist/skills/git/references/` (six **reference-module sources** that fan out into 42 files
...
| MDS name manifest | `tests/fixtures/mds-manifest.ts` | `MDS_COMMAND_HOSTS` (13), `MDS_PARTIALS` (12), `MDS_GENERATOR_HOSTS` (`['git']`), `MDS_REFERENCE_MODULES` (6 source paths, including the provider-independent `_pr.mds` and the gated `_mcp.mds`), `ALL_MDS_HOSTS` (14 — filenames only, excludes reference modules by construction), `ALL_DISCOVERED_HOSTS` (20 = 13 command + 1 generator + 6 reference-module sources — everything `output-dir:` finds, gated or not), `DIST_COMMAND_FILES` (14, incl. hand-authored `release.md`) — the single named-set source every count-literal test compares against, in both directions |
...
five unconditionally-registered entries plus the one gated entry declare it today. `_pr.mds`
sits exactly AT the floor — its `PR_HOST_OPS` roster is 8 entries, `MIN_VARIANT_PAIRS`
exactly — so it is the one module where removing a single op refuses the build outright
rather than merely shrinking a discriminating margin; the three provider modules (10 ops
each) and `_references.mds` (3 ops, but exempt as `'named'`) all carry headroom `_pr.mds`
does not.

**`_pr.mds` imports nothing on purpose (avoids PF-073)**: `@mdscript/mds`'s resolver deep-clones the entire definition-site scope into every `@define` — compile cost is exponential in a module's own `@define` count and multiplicative in the capture graph of anything it selectively imports, not linear in body text. `_pr.mds` is a plain fanout module — one `@define` per op, eight `<!-- op: -->` markers, no `@import` of `_tracker.mds` or `_common.mds` — precisely because its eight ops have nothing to share with the tracker providers; importing a heavier module the way `_jira.mds` imports `_mcp.mds` and `_common.mds` (measured seconds, not milliseconds, per PF-073) would tax every one of the ~52 full builds the test suite spawns per run for a module that does not need the shared tracker prose. Trimming an `@define` body would not help — the cost is the count of cloned `FunctionDef` nodes, not the text inside them.

**The build's manifest and the install's manifest are different numbers, on purpose (D-INSTALL-SET)**: `generatedReferenceManifest()` names every file the *resolved* registry emits — 42 today — because the tarball must serve any tracker selection without a rebuild. `installedReferenceManifest({ provider })` narrows that to what one install actually carries: the cross-cutting docs (`''`), the GitHub tree (`tracker/github`, the floor under every provider — GitHub's PR-review mechanics stay reachable even on jira/linear), the selected provider's own tree, and — since the PR-host split — `PR_HOST_DESTINATION_ROOT` (`'pr'`) unconditionally, because pull requests, PR reviews and PR checks stay on GitHub whatever the issue tracker is (applies ADR-026). Reading `generatedReferenceManifest()`'s count where the install footprint is meant is exactly the mistake ADR-027's 2026-09-19 STATUS audit caught (a CHANGELOG footprint claim measured against the wrong number); ADR-026 is what makes the corrected claim true by construction. The two numbers answer different questions and neither substitutes for the other.
