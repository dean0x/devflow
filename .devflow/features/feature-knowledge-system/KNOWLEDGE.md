...
| Build script | `scripts/build-mds.ts` | Frontmatter-driven: walks the whole repo (minus `IGNORE_DIRS`) for `.mds` files declaring a non-empty `output-dir:`, validates the destination + emitted filename via `src/core/mds-variants.ts`, and compiles each to `{output-dir}/{name}.md` (or `{output-name}.md`); refuses two hosts claiming one destination; hard-fails on any error |
