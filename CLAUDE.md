...
**Working Memory**: Three shell-script hooks (`scripts/hooks/`) replace the old 8-hook system with a sidecar architecture. Toggleable via `devflow memory --enable/--disable/--status` or `devflow init --memory/--no-memory`. Feature state is stored in `.memory/.sidecar/config.json` (primary source of truth); runtime sentinels (`.memory/.working-memory-disabled`, `.memory/.learning-disabled`) provide defense-in-depth for hooks that read files directly. `sidecar-capture` (Stop hook) — captures user/assistant turns to `.memory/.pending-turns.jsonl` queue and writes `.memory/.sidecar/memory.json` marker when throttle has expired (>2 min); uses atomic temp+mv for marker writes and PID-unique temp files to prevent concurrent clobbering. `sidecar-dispatch` (UserPromptSubmit hook) — captures user turn to queue, then scans `.memory/.sidecar/` for pending marker files; supports per-session marker filenames (`{type}.{session_id}.json`) for concurrency safety, deduplicates types before injecting SIDECAR directive, expires markers older than 24h, and uses `json_field_file` (node fallback) for timestamp reads. `sidecar-evaluate` (SessionEnd hook) — evaluates whether to trigger learning batching, decisions detection, or knowledge refresh; writes per-session marker files (e.g., `learning.{session_id}.json`) using atomic temp+mv; uses mkdir-based locking (`sidecar-lock`) to serialize reinforcement and batch-trigger operations across concurrent sessions; caps `.learning-sessions` at 50 lines to bound file growth. SessionStart hook (`session-start-memory`) → injects previous memory + git state as `additionalContext` on `/clear`, startup, or compact; gated by `.working-memory-disabled` sentinel. Always-on SessionStart hook (`session-start-context`) → injects cross-feature context (decisions TL;DR, learned behaviors); sections gated by both sidecar config and sentinels. PreCompact hook → saves git state + WORKING-MEMORY.md snapshot. Updates `.memory/WORKING-MEMORY.md` with structured sections (`## Now`, `## Progress`, `## Decisions`, `## Modified Files`, `## Context`, `## Session Log`). Disabling memory removes the three sidecar hooks (session-start-context is always-on and never removed). `devflow memory --disable` also removes pre-sidecar legacy hooks (`prompt-capture-memory`, `stop-update-memory`, `stop-update-learning`, `session-end-learning`, `session-end-decisions`, `session-end-knowledge-refresh`) from upgrading users. Use `devflow memory --clear` to clean up pending queue files across projects. Zero-ceremony context preservation.
...
├── .learning-disabled        # Runtime sentinel — session-end-learning hook exits immediately if present
