...
**Ambient Mode**: Two hooks provide always-on intent classification. SessionStart hook (`session-start-classification`) reads the installed router SKILL.md (`~/.claude/skills/devflow:router/SKILL.md`), strips frontmatter, and injects the classification rules as `additionalContext` — once per session, deterministic, zero model overhead. UserPromptSubmit hook (`preamble`) injects a one-sentence classification prompt per message. The router SKILL.md is the single source of truth for classification rules (intents, depths, skill selection tables, edge cases). Toggleable via `devflow ambient --enable/--disable/--status` or `devflow init`.
