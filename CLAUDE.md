...
**Decisions pipeline** (SessionStart-spawned detached worker — no markers, no subagent): `capture-prompt`/`capture-turn`/`capture-question` (all always-on) append every user turn, assistant turn, and answered `AskUserQuestion` to `.devflow/dream/.pending-turns.jsonl`, independently gated by the `decisions` field in dream config and the `.devflow/decisions/.disabled` sentinel (dual-signal). `spawn-dream-worker` (SessionStart, always-on) — when the dream queue is non-empty (or a leftover `.processing` batch exists) and `claude` is on PATH, spawns `background-dream-update` as a detached `nohup` worker. `background-dream-update` — claims the queue (rename-to-claim), resolves the model (project `decisions.json` → global `~/.devflow/decisions.json` → `opus` default), and spawns `claude -p` pointed at `scripts/hooks/dream-procedure.md` (a plain instruction file the agent reads via its own Read tool — not a Claude Code skill, since skills don't load in `claude -p` sessions). The agent does all detection and curation itself and calls the ledger ops directly via its own Bash tool: **decision**/**pitfall** detection via `assign-anchor` (internally self-locks `.decisions.lock`; assigns the next ADR-NNN/PF-NNN anchor number into `decisions-ledger.jsonl`, then deterministically renders `decisions.md`/`pitfalls.md` from the ledger — active entries only) and periodic curation via `retire-anchor` (flips `decisions_status`, never deletes) plus `rotate-observations`. Raw observations accumulate in the gitignored `.devflow/decisions/decisions-log.jsonl` (rotated to `decisions-log.archive.jsonl`). No deterministic thresholds or confidence formulas — the LLM determines whether an observation warrants a new entry or should be merged with an existing one. On success the agent touches `.devflow/dream/.last-dream-ok` (the worker's success signal) and optionally writes a 1-3 line `last-run-summary` that `session-start-context` injects once, then deletes. Global config: `~/.devflow/decisions.json`. Project config: `.devflow/decisions/decisions.json` (`model` and `debug` only — no daily-run cap or throttle; the worker is gated by queue non-emptiness instead). Runtime sentinel: `.devflow/decisions/.disabled` — the decisions sections in `session-start-context` and the dream queue gate skip if present; `devflow decisions --enable` removes it, `devflow decisions --disable` creates it. Toggleable via `devflow decisions --enable/--disable/--status` or `devflow init --decisions/--no-decisions`. Management subcommands: `devflow decisions list`, `devflow decisions --configure`, `devflow decisions --clear/--reset` (both resolve the git root explicitly and skip a project entirely while `.devflow/dream/.worker.lock` is held).
...
- `devflow decisions --enable/--disable` — Decisions pipeline (decision + pitfall detection, materialized by the detached dream worker from the captured queue)
...
│   └── hooks/              # Capture + memory + dream + ambient hooks (capture-prompt, capture-turn, capture-question, queue-append, memory-worker, background-memory-update [Stop-hook worker], spawn-dream-worker, background-dream-update [SessionStart-hook worker], dream-procedure.md, dream-lock, session-start-memory, session-start-context, pre-compact-memory, preamble, get-mtime, hook-bootstrap, hook-log-init)
...
├── dream/                        # config.json (feature toggles), .pending-turns.jsonl/.processing (decisions detection queue), .last-dream-ok (worker success stamp), .worker.lock/ (900s stale-break), last-run-summary (optional, agent-written, injected once by session-start-context then deleted)
├── decisions/
...
│   ├── decisions.md              # Architectural decisions (ADR-NNN) — rendered from decisions-ledger.jsonl (active only) by the detached dream worker via assign-anchor + render-decisions
│   ├── pitfalls.md               # Known pitfalls (PF-NNN, area-specific gotchas) — rendered from decisions-ledger.jsonl (active only) by the detached dream worker via assign-anchor + render-decisions
...
└── .background-dream-update.log       # background-dream-update worker log
```
...
**Model Strategy**: Explicit model assignments in agent frontmatter override the user's session model. Opus for analysis agents (reviewer, scrutinizer, evaluator, designer, researcher, bug-analyzer), Sonnet for execution agents (coder, simplifier, resolver, skimmer, tester, knowledge), Haiku for I/O agents (git, synthesizer, validator). Dream is no longer a Claude Code agent at all — it is a detached `claude -p` worker (`background-dream-update`, spawned by the `spawn-dream-worker` SessionStart hook), defaulting to opus (project `decisions.json` → global `~/.devflow/decisions.json` → `opus` default) for decisions detection and curation. Memory is refreshed the same way: the detached `background-memory-update` worker (`claude -p --model haiku`), spawned by the `memory-worker` Stop hook. Knowledge is not a background worker — the Knowledge agent (sonnet) is spawned in-command by `knowledge_writeback()` at workflow end.
