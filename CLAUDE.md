...
│   ├── cli/                # CLI command modules (init, init-seed, install-report, uninstall, ambient, learning, flags, knowledge, rules, debug, hud, proxy, agents, compliance, tracker)
...
│   ├── targets/claude-code/ # Claude Code install target (installer, tracker-install, compliance-install, hooks.ts, post-install, claude-paths, legacy, templates/)
...
│       ├── mds/            # MDS reference modules (tracker/_common.mds and tracker/_github.mds always, tracker/_mcp.mds + _jira.mds + _linear.mds generation-gated, git/_references.mds and git/_pr.mds → dist/skills/git/references/)
...
│   ├── installer/          # Generated-reference overlay (converge-not-merge, atomic per-unit swap) and the scoped install shape
