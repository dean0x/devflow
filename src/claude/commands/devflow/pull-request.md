...
If push fails or branch not on remote:
```bash
# Check remote exists
git remote -v
...
### "PR already exists"
If PR exists but you want to update description:
```bash
# Update existing PR description manually
gh pr edit <number> --body "New description"

# Or close and recreate (destructive!)
gh pr close <number>
/pull-request
```

**Note**: Pushing new commits to the branch automatically updates the existing PR.

### "Sub-agent failed to generate description"
If PR description generation fails:
```bash
# Manual PR creation with browser
gh pr create --web  # Opens browser for manual entry

# Or use minimal description
gh pr create --title "Your title" --body "See commits for details"
```
...
/pull-request main  # or master, develop, etc.
```

### "No commits ahead of base"
If branch has no new commits:
```bash
# Check commit status
