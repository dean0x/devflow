...
> **Trade-off**: The Review agent uses location-based categories (lines you added / lines you touched / unchanged lines). Diagnose agent focuses on diff-changed code and lacks per-line location context, so it approximates using severity as a proxy. This means a LOW-severity bug in newly-added code is placed in Pre-existing — not because it predates the change, but to signal lower urgency. The resolve pipeline should treat Pre-existing findings from Diagnose agent as low-urgency, not as assertions about code origin.
