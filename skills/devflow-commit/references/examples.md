# Commit Examples - Extended Reference

Extended examples for atomic commit grouping and message formatting.

---

## Atomic Commit Grouping - Extended Examples

### Feature Implementation (Multi-Commit Flow)

When implementing a new feature, split changes logically:
...
After completing atomic commits, verify the commit history before creating PR:
...
# Create PR (assuming remote is set)
gh pr create --title "feat(users): implement user management" --body "$(cat <<'EOF'
## Summary
- Add UserService with CRUD operations
- Add REST API endpoints for users
