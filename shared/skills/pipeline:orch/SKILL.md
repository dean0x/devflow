...
`FEATURE_KNOWLEDGE` loading is handled by each sub-orchestrator (implement:orch Phase 2, review:orch Phase 3, resolve:orch Phase 2). Pipeline:orch does NOT load feature knowledge itself — it delegates to the inner skills which handle loading, staleness checks, and agent distribution independently.
...
Cleanup: delete `.devflow/docs/handoff-{branch_slug}.md` if it exists (implement:orch Phase 8 normally removes it; this is a safety net before proceeding to review).
...
Load `devflow:review:orch` via the Skill tool, then execute its full pipeline (Phases 1-7: pre-flight → incremental detection → decisions index → file analysis → parallel reviewers (7 core + conditional) → synthesis → finalize). All 7 core reviewers (security, architecture, performance, complexity, consistency, testing, regression) are mandatory.
