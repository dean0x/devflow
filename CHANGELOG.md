...
- **`devflow agents` TUI — four defects fixed** (`fix/agents-tui` wave, PR 3 of 4):
  ...
  - **Fix 3 (install state + orphan visibility)**: `AgentRow` and `InitRowInput` gain required `installed` and `inRegistry` fields. `buildTuiState` calls `readInstalledAgentNames` (one `readdir`, no per-agent `fs.access`) and appends orphan rows — arbitrary keys from `agent-models.json` not present in the registry — with `inRegistry: false`. A 4th **STATE** column is added to the TUI frame (AGENT 18, MODEL 32, EFFORT 13, STATE 13 = 78 ≤ 80), showing `active` / `saved-inactive` / `not installed` / `unknown`. `stripAnsi(row.name)` is mandatory before name-cell rendering to prevent ANSI injection via hostile JSON keys.
  - **Fix 4 (capitalized names)**: `formatAgentName` added to `render.ts` with exactly ONE call site — capitalizes the first character of the agent name for TUI display only. `--list` output remains lowercase.
