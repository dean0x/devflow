...
- **`fetch-issue` and `fetch-issues-batch` rejected `#`-prefixed issue references** — before: a `#`-prefixed reference (e.g. `#42`) was routed to the search path instead of the direct-fetch path, fetching no issue in normal cases. After: a leading `#` is stripped before the numeric/text branch (`#42` ≡ `42`). (AC-0.3)
